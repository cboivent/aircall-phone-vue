...
export default class HomePage extends Vue {
  ...
  onUpdatedCall(call: Call): void {
    console.log(`[HomePage][Subscription] - receive update on call ${call.id} - ${call.created_at}`, call)
    ...
    if (indexInGroupList > -1) {
      console.log(`[HomePage] - found call on grouped list at position ${indexInGroupList}`)
      ...
      if (indexInCallsList > -1) {
        console.log(`[HomePage] - found call on calls list at position ${indexInCallsList} on group #${indexInGroupList}`)
        ...
        console.log(`[HomePage] - not found call ${call.id} in calls list on group #${indexInGroupList}`)
      }
    } else {
      console.log(`[HomePage] - not found call ${call.id} in group list`)
    }
  }

  async mounted(): Promise<void> {
    // subscribe to on updatedCall
    console.log('[HomePage] start subscription to updated call')
    ...
    console.log('[HomePage] nb calls', this.totalCount)
    // get all calls id
    ...
    console.log(`[HomePage] found ${this.itemsGroupedByDate.length} group(s)`)
    //
    this.loading = false
  }

  async clickOnGroup(index: number): Promise<void> {
    console.log('[HomePage] click on group', index)
  ...
  async showCall(id: number): Promise<void> {
    console.log('[HomePage] receive click on call', id)
    this.$router.push(`/call/${id}`)
  }

  async archiveCall(id: number): Promise<void> {
    console.log('[HomePage] receive archive call event for call', id)
